 ...
 * @return array
 */
function virgo_get_style_color() {
 ...
 * @return bool|string
 */
function virgo_enqueue_url_base_style() {
	global $wp_filesystem;
	if ( empty( $wp_filesystem ) ) {
		require_once( ABSPATH . '/wp-admin/includes/file.php' );
		WP_Filesystem();
	}
	$virgo_upload_dir = wp_upload_dir();
	$virgo_filename   = trailingslashit( $virgo_upload_dir['basedir'] ) . 'style.css';
	if ( $wp_filesystem->exists( $virgo_filename ) ) {
		$url = $virgo_upload_dir["baseurl"];
...
add_action( 'widgets_init', 'virgo_widgets_int_plugin' );


function virgo_widgets_int_plugin() {

	//sidibar widget
	register_widget( 'virgo_twiter_Wigdet' );


}

/**
 * Class virgo_twiter_Wigdet
 */
class virgo_twiter_Wigdet extends WP_Widget {
	function __construct() {
		$args = array(
			'name'        => esc_html__( 'virgo  Tweets', 'virgo' ),
			'description' => esc_html__( 'It displays a list of tweets', 'virgo' ),
			'classname'   => 'virgo_twiter'
		);
		parent::__construct( 'virgo_twiter', esc_html__( 'virgo Tweets', 'virgo' ), $args );
	...
	function widget( $args, $instance ) {
		...
		$instance = wp_parse_args(
			(array) $instance,
			array(
				'title' => esc_html__( 'TWEETS', 'virgo' ), // Legacy.
				'Name'  => '',
				'text'  => 3
							...
							<div class="media-body">
								<p><i class="fa fa-twitter"></i>
									...
									<small><?php
										...
										printf( '%s ' . esc_html__( 'ago', 'virgo' ), esc_html( $human_time ) );
										?>
									</small>
							</div>
						</div>
