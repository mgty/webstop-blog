...
/* Write your awesome functions below */
	function virgo_arphabet_widgets_init() {
		register_sidebar( array(
			'name' => esc_html__( 'sidebar', 'webstop_virgo' ),
			'id' => 'virgo_sidebar',
			'before_widget' => '<div id="%1$s" class="widget   %2$s">',
			'after_widget' => '</div>',
			'description' => esc_html__( 'blog sidebar', 'webstop_virgo' ),
			'before_title' => '<h3 class="widget-title">',
			'after_title' => '</h3>',
		) );

		register_sidebar( array(
			'name' => esc_html__( 'Footer sidebar ', 'webstop_virgo' ),
			'id' => 'virgo_footer_sidebar',
			'before_widget' => '<div id="%1$s" class="col-base col-sm-6 col-md-3 widget   %2$s">',
			'after_widget' => '</div>',
			'description' => esc_html__( 'Footer sidebar', 'webstop_virgo' ),
			'before_title' => '<h3 class="widget-title">',
			'after_title' => '</h3>',
		) );

		register_sidebar( array(
			'name' => esc_html__( 'Footer Dark sidebar ', 'webstop_virgo' ),
			'id' => 'virgo_footer_sidebar_dark',
			'before_widget' => '<div id="%1$s" class="col-base col-sm-6 col-md-3widget   %2$s">',
			'after_widget' => '</div>',
			'description' => esc_html__( 'Footer Dark sidebar', 'webstop_virgo' ),
			'before_title' => '<h3 class="widget-title">',
			'after_title' => '</h3>',
		) );

		register_sidebar( array(
			'name' => esc_html__( 'Footer brown sidebar ', 'webstop_virgo' ),
		...
		register_sidebar( array(
			'name' => esc_html__( 'Footer gray sidebar ', 'webstop_virgo' ),
...
add_action( 'wp_enqueue_scripts', 'webstop_virgo_style_scripts', 500 );


/**
 *
 */
function webstop_virgo_style_scripts() {
	...
	if ( is_page() || is_single() ) {
		...
		if ( get_page_template_slug( get_the_ID() ) == "template-architecture.php" ) {
			...
			wp_enqueue_style( 'virgo-fonts-Libre', $virgo_class->virgo_google_fonts_url( 'Libre+Baskerville:400,400i,700,700i,900' ) );
			wp_enqueue_style( 'virgo-fonts-Monsterrat', $virgo_class->virgo_google_fonts_url( 'Montserrat:400,400i,700,700i' ) );
		}
	...
	//************************************* Fonts ***********************************************/
	wp_enqueue_style( 'virgo-fonts-google-Roboto', $virgo_class->virgo_google_fonts_url( 'Libre+Baskerville:400,400i,700,700i|Montserrat:400,700|Libre+Baskerville:400,400i,700,700i' ) );
 ...
 */
function webstop_virgo_style_scripts_admin() {
	//Geocoding google
...
add_action( 'admin_enqueue_scripts', 'webstop_virgo_style_scripts_admin' );
...
function virgo_limit_excerpt( $limit, $content = null ) {
	...
	$excerpt .= '<a class="moretag" href="'. get_permalink($post->ID) . '">...(Read Full Article)</a>';
